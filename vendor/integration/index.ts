  ...
  return {
    ...
    hooks: {
      'astro:config:setup': async ({
        ...
        // Keep the production GitHub Pages base path, but use a clean root URL
        // during local development so links work from http://localhost:4321/.
        const isDev = command === 'dev';
        const runtimeSITE = {
          ...SITE,
          site: isDev ? `http://localhost:${config.server?.port ?? 4321}` : SITE.site,
          base: isDev ? '/' : SITE.base,
        };

        updateConfig({
          site: runtimeSITE.site,
          base: runtimeSITE.base,

          trailingSlash: runtimeSITE.trailingSlash ? 'always' : 'never',
            ...
            plugins: [
              {
                ...
                load(id) {
                  if (id === resolvedVirtualModuleId) {
                    return `
                    export const SITE = ${JSON.stringify(runtimeSITE)};
